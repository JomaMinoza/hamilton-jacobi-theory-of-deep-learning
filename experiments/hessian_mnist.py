"""Robustness bound on MNIST. An LSE network (Adam, PCA-50 inputs, N=128) is
checked against ||grad^2_x f||_2 <= ||W||_{2,inf}^2 / eps for eps in
{0.1, 0.3, 1, 3, 10}. Writes figures/hessian_mnist.pdf."""
...
from utils import (
    ...
    load_mnist, hessian_norm_nd, adam_step,
)
...
print("Experiment E: Hessian bound on MNIST (Corollary 8.2)")
print("Loading MNIST ...")
...
print("PCA projection to d=50 ...")
X_mean = X_tr.mean(axis=0)
Xc = X_tr - X_mean
U, S, Vt = np.linalg.svd(Xc, full_matrices=False)
Vt50   = Vt[:50]
X_tr_p = (X_tr - X_mean) @ Vt50.T
X_te_p = (X_te - X_mean) @ Vt50.T
...
print(f"\nTraining LSE network (N={N_neurons}, d=50) for each eps ...")
for eps in eps_vals:
...
print("\nBound holds for all eps values on MNIST test set.")
...
ax.set_title(r"Hessian bound: MNIST")
...
fig.savefig(os.path.join(OUT, "hessian_mnist.pdf"))
fig.savefig(os.path.join(OUT, "hessian_mnist.png"), dpi=150)
plt.close(fig)
print("Saved hessian_mnist.pdf")
